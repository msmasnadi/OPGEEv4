...
The ``opg`` script can be extended using plug-ins, i.e., python modules that
are loaded on demand into opg. The ``subcommand`` module provides
...
"plugin path" set using the config file parameter ``OPGEE.PluginPath``. For
example, you might create a file ``foo_plugin.py`` in the directory ``~/plugins``.
You would then set ``OPGEE.PluginPath = %(Home)s/plugins`` in the ``~/opgee.cfg``
configuration file.

Plugins are loaded by ``opg`` on demand, i.e., when the plugin is referenced
on the ``opg`` command-line, or implicitly when all steps are run. They are
...
``opgee/etc/pluginTemplate.py``, which contains the code shown here.

.. literalinclude:: ../../opgee/etc/pluginTemplate.py
   :language: python

API
---

.. automodule:: opgee.subcommand
   :members:
