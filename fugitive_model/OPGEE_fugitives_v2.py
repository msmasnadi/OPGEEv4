...
def lookup_func(field_mean):
    ...
    tmp = \
        prod_mat_gas[
            (prod_mat_gas['Bin low'] < field_mean) & (prod_mat_gas['Bin high'] >= field_mean)].index.values.astype(
            int)[0]
...
# (1) Approximate field productivity distribution
...
Field_Productivity['Assignment'] = Field_Productivity.apply(
    lambda row: lookup_func(row['Mean gas rate (Mscf/well/day)']), axis=1)
