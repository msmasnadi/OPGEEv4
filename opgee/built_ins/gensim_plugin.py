...
def genFullFactorialData(trials, paramFileObj, args):
    ...
    for elt in paramFileObj.tree.iterfind('//Parameter'):
        ...
        if distName not in supported_distros:
            raise McsUserError("Found '{}' distribution; must be one of {} for use with SALib.".format(distName, supported_distros))
    ...
    if trials % N != 0:
        raise McsUserError(f"Trial count {trials} is not an exact multiple of the number of full-factorial combinations ({N})")
...
def genTrialData(simId, trials, paramFileObj, args):
    ...
    from opgee.mcs.distro import linkedDistro
    from opgee.mcs.LHS import lhs, lhsAmend
    ...
    # TBD: on integration. (getName() will fail on XMLVariable instances)
...
#     """
#     import ..tool
#     from ..utils import mkdirs, symlink
...
def genSimulation(simId, trials, paramPath, args):
    ...
    from ..util import getSimParameterFile, getSimResultFile
    from ..constants import LOCAL_XML_NAME
    from ..project import Project
    from ..xmlSetup import ScenarioSetup
    from ..utils import symlink, filecopy
...
def _newsim(runWorkspace, trials):
    ...
    if not runWorkspace:
        raise McsUserError("MCS.RunWorkspace was not set in the configuration file")
...
def _exportVars(paramFile, args):
    ...
    from .mcs.XMLParameterFile import XMLDistribution, XMLParameterFile
...
def driver(args, tool):
    ...
    if trials < 0:
        raise McsUserError("Trials argument is required: must be an integer >= 0")
    ...
    if runRoot:
        ...
        _logger.info(f'Please add "MCS.Root = {runRoot}" to your opgee.cfg file in the [{projectName}] section.')

    runDir = getParam('MCS.RunDir', section=projectName)

    if args.delete:
        removeTree(runDir, ignore_errors=False)
