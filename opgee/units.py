...
.. Created as part of pygcam based on gcam-data-system/_common/assumptions/unit_conversions.R
...
Use ``getUnits()`` to get the singleton instance of the ``UnitNamespace`` class.
For example,

  .. code-block:: python

     from opgee.units import getUnits
...
class UnitNamespace(object):
    ...
    unit conversions, allowing these names to be used in the "chart"
    sub-command to specify values to multiply or divide by to convert
    units for plotting. Names can be set and accessed using
    ...
    def get(self, name, raiseError=True):
        ...
        try:
            ...
            if raiseError:
                raise OpgeeException('Unknown unit conversion string "%s"' % name)
            return None
