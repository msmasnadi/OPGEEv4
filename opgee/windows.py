...
if IsWindows:
    ...
    def get_read_handle(filename):
        ...
        try:
            ...
            raise OpgeeException("get_read_handle(%s) failed: %s" % (filename, e))
    ...
    def symlinkWindows(src, dst):
        ...
        if samefileWindows(src, dst):
            raise OpgeeException("Attempted to create symlink loop from '%s' to '%s' (same file)" % (dst, src))
        ...
        try:
            ...
            if getParamAsBoolean('OPGEE.SymlinkWarning'):
                _logger.error('''
  ============================================================================================================
  WARNING: The current user does not have permission to create symbolic links, forcing opgee to copy rather
  than symlink files. This uses much more file space than using symlinks, but it works.  To use opgee more
  ...
  Set "OPGEE.SymlinkWarning = False" in ~/opgee.cfg to suppress this message.
  ============================================================================================================
  ''')
            raise OpgeeException("Failed to create symlink '%s' to '%s': %s" % (dst, src, e))
    ...
    def islinkWindows2(path):
        ...
        if result == INVALID_FILE_ATTRIBUTES:
            raise OpgeeException("Can't get file attributes for '%s': %s'" % (path, ctypes.WinError()))
...
def removeSymlink(path):
    ...
    if not os.path.islink(path):
        raise OpgeeException("removeSymlink: path '%s' is not a symlink" % path)
