...
def test_gas_density(gas_instance, stream):
    ...
    assert density == ureg.Quantity(pytest.approx(0.069268389), "tonne/m**3")


def test_gas_viscosity(gas_instance, stream):
    viscosity = gas_instance.viscosity(stream)
    assert viscosity == ureg.Quantity(pytest.approx(0.0172078219), "centipoise")
...
def test_gas_volume_flow_rate(gas_instance, stream):
    ...
    assert vol_flow_rate == ureg.Quantity(pytest.approx(1588.62879), "m**3/day")
...
def test_volume_energy_density(gas_instance, stream):
    ...
    assert volume_energy_density == ureg.Quantity(pytest.approx(955.778111), "btu/ft**3")
